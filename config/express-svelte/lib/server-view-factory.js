...
const VIEW_GLOBALS_COMPONENT_FILENAME =
  __dirname + "/components/ViewGlobals.svelte";

const ERROR_EXPRESS_SVELTE_COMPILE_OUTPUT_INVALID_LENGTH =
  "ERROR_EXPRESS_SVELTE_COMPILE_OUTPUT_INVALID_LENGTH";
const ERROR_EXPRESS_SVELTE_COMPILE_OUTPUT_INVALID_TYPE =
  "ERROR_EXPRESS_SVELTE_COMPILE_OUTPUT_INVALID_TYPE";
...
class ServerViewFactory {
  ...
  static async compile(wrappedViewFilename, opts = {}) {
    ...
    const dev =
      opts.dev != null
        ? opts.dev
        : env === NODE_ENV_DEVELOPMENT || env === NODE_ENV_TESTING;
    ...
    if (output.length !== 1) {
      const err = new Error(
        `Invalid compile output. Generated more than one chunk or asset. wrappedViewFilename:${wrappedViewFilename} output.length:${output.length}`
      );
    ...
    if (chunk.type !== "chunk") {
      const err = new Error(
        `Invalid compile output. Generated an asset instead of a chunk. wrappedViewFilename:${wrappedViewFilename}`
      );
