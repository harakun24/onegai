...
class BrowserViewFactory {
  ...
  static async generateSource(relativeFilename, wrappedAbsoluteFilename) {
    const viewGlobalsRelativePath =
      path.relative(
        path.dirname(wrappedAbsoluteFilename),
...
new ViewGlobals({
  ...
  static async createWrappedView(absoluteFilename, output) {
    const wrappedAbsoluteFilename = path.join(
    ...
    const source = await this.generateSource(
      relativeFilename,
      wrappedAbsoluteFilename
    );
  ...
  static async create(input, opts = {}) {
    ...
    for (let i = 0; i < absoluteFilenames.length; i++) {
      ...
      const output = _getOutputFilename(
        absoluteFilename,
        opts.relative || DEFAULT_RELATIVE
      );

      // Push modern config
      let wrappedAbsoluteFilename = await this.createWrappedView(
        absoluteFilename,
        output
      );
      let completeLegacyConfig = this.getConfig({
        ...opts,
        input: wrappedAbsoluteFilename,
      ...
      // Push legacy config
      if (legacy === true) {
        wrappedAbsoluteFilename = await this.createWrappedView(
          absoluteFilename,
          output
        );
        completeLegacyConfig = this.getConfig({
          ...opts,
          input: wrappedAbsoluteFilename,
